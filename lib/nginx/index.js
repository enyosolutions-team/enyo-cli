...
function initNginxSite(name, options, config) {
  ...
  return new Promise((resolve, reject) => {
    if (!options.git) {
      reject('Missing repository'.red);
      console.log('Missing repository'.red);
    }
    ...
    findPort().then((port) => {
      ...
      switch (options.type) {
        ...
        console.log('')
        console.log('')
        console.log('TYPOE NOT FOUND', options.type)
        console.log('TYPOE NOT FOUND', options)
      }
      ...
      if (options.reload) {
        ...
        resolve({status:'ALL DONE', port: appPort});
        return;
      }
      resolve({status:'ALL DONE', port: appPort});
