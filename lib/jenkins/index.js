...
const createJob = (name, options, config) => {
  ...
  return new Promise((resolve, reject) => {
    jenkins.job.get(name).then((data) => { // try to find and existing job
      console.log('Job already exists');
      resolve({ errors: [{ message: 'Job already exits' }] });
    }).catch(err => Gitlab.get(`/projects/${options.gitProjectId}/repository/files/readme.md?ref=master`))
      .then((output) => {
      ...
      .then((xml) => { // change the configuration to point to the right repository.
        ...
        return jenkins.job.create({ name, xml });
      })
      .then(data => jenkins.job.build(name))
      .then((data) => {
