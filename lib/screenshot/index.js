...
module.exports = function screenshot(url, argv = {}){
  ...
  return  new Promise((resolve, reject) => {
   ...
   mkdirp(targetFolder, function (err) {
    ...
    promises = resolutions.map(function(res) {

      let nightmare =  Nightmare({show: false, frame: false, maxHeight:4000, maxWidth:4000, width: res.width, height: res.height });
      ...
      nightmare.viewport(res.width, res.height).goto(url).wait('body').evaluate(() => {
        ...
        if(argv.pdf){
         ...
         nightmare.pdf(targetFolder + '/screenshot_'+ res.name + res.width + dimensions.height + '.pdf',{pageSize:dimensions}).end();
       }
       else {
        if (argv.verbose)
          console.log('Creating png screenshot', targetFolder + '/screenshot_'+ res.name +'.png', dimensions);

        nightmare.screenshot(targetFolder + '/screenshot_'+ res.name +'.png').viewport(dimensions.width, dimensions.height)
        .wait(500)
        .screenshot(targetFolder + '/screenshot_'+ res.name +'_full.png').end();
      }
