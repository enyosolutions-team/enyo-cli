...
const createRepository = function (repoName, options, config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(() => {
      ...
      const p2 = gitApi
      .post(`/projects/${repoId}/repository/branches`, { branch: 'wip', ref: 'master' });
    ...
    .then((promiseResults) => {
      ...
      if (options.ci) {
        return Promise.all([
                           gitApi.post(`/projects/${repoId}/hooks`, {
                            url: `https://ci.dev.enyosolutions.com/project/${repoName}.wip?token=enyosolutions.ci`, push_events: true, tag_push_events: true, merge_requests_events: true
                          }),
                           gitApi.post(`/projects/${repoId}/hooks`, {
                            url: `https://ci.dev.enyosolutions.com/project/${repoName}.dev?token=enyosolutions.ci`, push_events: true, tag_push_events: true, merge_requests_events: true
                          }),
                           gitApi.post(`/projects/${repoId}/hooks`, {
                            url: `https://ci.dev.enyosolutions.com/project/${repoName}.staging?token=enyosolutions.ci`, push_events: true, tag_push_events: true, merge_requests_events: true
                          }),
    ...
    .then((res) => { resolve(finalOutput) && console.log('Hook created'.green); })
    .catch((err) => {
...
const addWebhook = function (repoName, hookUrl, options, config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((response) => {
      ...
      return gitApi.post(`/projects/${repoId}/hooks`, {
        url: `https://ci.dev.enyosolutions.com/project/${repoName}.dev`, push_events: true, tag_push_events: true, merge_requests_events: true
      });
    })
...
module.exports = { createRepository, addWebhook };
